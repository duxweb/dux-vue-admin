...
// node_modules/@alova/adapter-axios/dist/alova-adapter-axios.esm.js
var mockResponseHandler = ({ status, statusText, body }, _, currentMethod) => {
  ...
  const axiosConfig = {
    ...
    ...config,
    headers: new AxiosHeaders2(config.headers)
  };
  const axiosResponse = {
    data: body,
    status,
  ...
  if (status >= 400) {
    throw new AxiosError2(statusText, "ERR_BAD_REQUEST", axiosConfig, undefinedValue, axiosResponse);
  }
  return {
...
var mockErrorHandler = (error, currentMethod) => {
  const { config } = currentMethod;
  return new AxiosError2(error.message, "ERR_NETWORK", {
    baseURL: currentMethod.baseURL,
    url: currentMethod.url,
    data: currentMethod.data,
    ...config,
...
function requestAdapter(options = {}) {
  const adaptedAxiosInstance = options.axios || axios_default;
...
export {
  mockResponse as axiosMockResponse,
  requestAdapter as axiosRequestAdapter
};
