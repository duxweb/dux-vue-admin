...
export const DuxLoginPage = defineComponent({
  ...
  setup(_props) {
    ...
    onSuccess((res) => {
      manage.login(resource?.manage as string, res.data?.data)
      router.replace(`/${resource.manage}`)
    })
            ...
            <div class="my-6">
              <NForm ref={formRef} model={form}>
                <NFormItem showLabel={false} path="username">
                  <NInput value={form.username} onUpdateValue={v => form.username = v} type="text" placeholder="请输入账号" size="large">
                    {{
                ...
                <NFormItem showLabel={false} path="password">
                  <NInput value={form.password} onUpdateValue={v => form.password = v} type="password" showPasswordOn="mousedown" placeholder="请输入密码" size="large" inputProps={{ autocomplete: 'new-password' }}>
                    {{
